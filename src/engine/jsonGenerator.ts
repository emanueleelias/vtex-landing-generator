 ...
 */
function wrapWithContainer(rootKey: string, containerSuffix: string, containerTitle: string, nodes: Record<string, any>) {
 ...
 */
function processBlock(block: Block, landingName: string, index: number, layout: 'desktop' | 'mobile'): { childKey: string; nodes: Record<string, any> } | null {
  const definition = getBlockDefinition(block.type)
  if (!definition) return null

  let result: { rootKey: string; nodes: Record<string, any> } | undefined
 ...
 */
export function generateLandingJSON(state: { landingName: string; desktopBlocks: Block[]; mobileBlocks: Block[] }): Record<string, any> {
  ...
  // --- DESKTOP ---
  const desktopChildren: string[] = []
  const desktopNodes: Record<string, any> = {}
  ...
  // --- MOBILE ---
  const mobileChildren: string[] = []
  const mobileNodes: Record<string, any> = {}
