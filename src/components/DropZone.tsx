...
export default function DropZone({ id, parentId, index, className, text, targetType = 'children' }: DropZoneProps) {
    ...
    if (className) {
        ...
        return (
            <div
                ref={setNodeRef}
                className={`transition-all ${className} ${isOver ? 'bg-teal-500/10 border-teal-500 text-teal-400' : ''
                    }`}
            >
                {text}
            </div>
    ...
    return (
        ...
        >
            <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-teal-500 -translate-y-1/2 rounded-full shadow-[0_0_8px_rgba(236,72,153,0.8)]" />
        </div>
    )
}
